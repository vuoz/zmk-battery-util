...
class BluetoothManager: NSObject, ObservableObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    func peripheral(
        ...
        where characteristic.uuid == batteryLevelCharacteristicUUID {
    ...
    func peripheral(
        ...
        guard let data = characteristic.value,
            let newReading = data.first
        else { return }

        print(
            "Packet received for peripheral \(peripheral.name ?? "Unknown") with value: \(data.first ?? 0)"
        )
        ...
        if batteryInfo[peripheral.identifier]?.levels.count == 0 {
            batteryInfo[peripheral.identifier]?.levels.append(
                Reading(level: newReading, ts: ts_curr))
        }
                    ...
                    ForEach(bluetoothManager.connectedDevices, id: \.identifier) { peripheral in
                        Button {
                            ...
                            HStack {
                                ...
                                Spacer()
                                Text(
                                    bluetoothManager.batteryInfo[peripheral.identifier]?.label
                                        ?? "N/A"
                                )
